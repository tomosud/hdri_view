...
- 選択中はコードと出力解像度をバージョン付きJSONからBase64URL化して `#glsl=...` に保持する。
- 編集中コード、解像度、プリセットの変更時は履歴を増やさずURLを更新し、共有URLから画像を再生成する。
- 元画像付きGLSL、Original、背景選択時は画素を共有対象にせず、フラグメントの無い通常URLへ戻す。
